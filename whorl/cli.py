...
def cmd_status(args):
    ...
    from .core.config import WHORL_DIR, DB_PATH, CONFIG_PATH
...
def cmd_scout_run(args):
    _boot()
    from .scouts import run_sweep
    run_sweep()


def cmd_scout_list(args):
    _boot()
    from .scouts import list_signals
...
def cmd_forge_pitch(args):
    _boot()
    from .forge import generate, print_pitch
...
def cmd_forge_list(args):
    _boot()
    from .forge import list_pitches
...
def cmd_seat(args):
    _boot()
    from .hotseat import run, print_history
    if args.topic:
        run(args.topic)
    else:
        print_history()
...
def cmd_loom_scan(args):
    # This integration surface is intentionally read-only until the
    # CodeCity-Bench adapter is wired. Do not initialize the DB or require
    # optional TOML configuration just to inspect the command surface.
...
def cmd_agent_yvette(args):
    _boot()
    from .agents.yvette import interactive_session
    from .core.models import Vertical
...
def cmd_tailor_qrd(args):
    _boot()
    from .tailor import qrd, print_qrd
    if not args.text:
...
def cmd_tailor_intent(args):
    _boot()
    from .tailor import parse_intent
    import json
    if not args.thought:
...
def cmd_vault_init(args):
    from .core.vault import init_interactive
    init_interactive()

def cmd_vault_status(args):
    from .core.vault import status
    status()

def cmd_vault_push(args):
    from .core.vault import sync_push
    sync_push(args.url)

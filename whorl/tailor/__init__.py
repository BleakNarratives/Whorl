...
# The Tailor's core dependencies (db/config/models) live in this package's
# own legacy core, whorl.whorl.core. Relative imports keep the workbench
# self-contained: the top-level `whorl` namespace belongs to the modern
# language package and is never touched. The QRD/db paths degrade to dict +
# no-op persistence if the legacy core is unavailable.
try:
    from ..core import db
    from ..core.config import cfg, load_api_keys
    from ..core.models import QRD
except Exception:
    db = None
    cfg = None
    load_api_keys = None
...
def qrd(text, source_id=""):
    ...
    if QRD is not None:
        record = QRD(**record)
    if db is not None:
        db.insert("qrds", record.__dict__ if hasattr(record, "__dict__") else record)
    return record

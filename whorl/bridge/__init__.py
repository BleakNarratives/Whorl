...
# The bridge is part of the LEGACY workbench (whorl.whorl), so its modules live
# under whorl.whorl.* — the top-level `whorl` namespace belongs to the modern
# language package and must never be resolved here (whorl.tailor, for example,
# would silently grab the modern module, which has a different API).
...
_hotseat = _try_import("whorl.whorl.hotseat")
_forge   = _try_import("whorl.whorl.forge")
_tailor  = _try_import("whorl.whorl.tailor")
_scouts  = _try_import("whorl.whorl.scouts")
_models  = _try_import("whorl.whorl.core.models")
_db      = _try_import("whorl.whorl.core.db")
...
class BoardroomHandler(BaseHTTPRequestHandler):
    ...
    def _handle_status(self):
        modules = {k.replace("whorl.whorl.", "legacy.").replace("whorl.", ""): v
                   for k, v in _WHORL_OK.items()}
        db_counts = {}
        if _db and _WHORL_OK.get("whorl.whorl.core.db"):
    ...
    def _handle_convene(self):
        ...
        enriched = brief
        if _tailor and _WHORL_OK.get("whorl.whorl.tailor"):
            try:
        ...
        if _hotseat and _WHORL_OK.get("whorl.whorl.hotseat"):
            try:
    ...
    def _handle_forge(self):
        ...
        if not target:
            ...
            return
        if not (_forge and _WHORL_OK.get("whorl.whorl.forge")):
            _json_response(self, {"error": "forge module unavailable"}, 503)
            return
        if not (_models and _WHORL_OK.get("whorl.whorl.core.models")):
    ...
    def _handle_tailor_qrd(self):
        ...
        if not text:
            ...
            return
        if not (_tailor and _WHORL_OK.get("whorl.whorl.tailor")):
    ...
    def _handle_scouts(self):
        if not (_scouts and _WHORL_OK.get("whorl.whorl.scouts")):
